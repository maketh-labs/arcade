...
contract MulRewardPolicyTest is Test {
    ...
    function testEscrow() public {
        ...
        uint256 amount = policy.escrow(0.1 ether, address(0), data);
        assertEq(amount, 0.2 ether);

        vm.expectRevert("MulRewardPolicy: toll is out of range");
        policy.escrow(0.09 ether, address(0), data);

        vm.expectRevert("MulRewardPolicy: toll is out of range");
        policy.escrow(1.01 ether, address(0), data);
        ...
        policy.escrow(0.1 ether, address(0), invalidData);
    }
